// Same-origin API client — see luv-and-ker's apiClient.ts for the full
// rationale. Server-side calls hit the proxy (`/api/...`) which forwards
// to the shared portal at PORTAL_API_ORIGIN. Browser-side calls hit the
// proxy directly so the `lk_session_v1` cookie flows.
